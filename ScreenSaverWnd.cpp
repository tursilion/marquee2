...
int CScreenSaverWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	SetWindowPos(&CWnd::wndNoTopMost, 0, 0, Width/3, myRect.bottom, SWP_NOMOVE);
#endif
...
{
#ifdef _DEBUG
	static bool fSawKey=false;

	// trap events to not stop the screen saver - until we see a keypress
	if (uMsg == WM_CHAR) fSawKey=true;
	if (!fSawKey) return true;
#endif
